...
| Subscriptions | ✅ **topic-based rest-hook** (R5 Backport IG — R4 has no native): topic + filter match on write → backport notification Bundle to the endpoint; handshake, `$status`, payload empty/id-only/full-resource; SSRF-guarded; opt-in (`FHIRENGINE_SUBSCRIPTIONS_ENABLED`). Single-node dispatch |
...
| MPI / dedup (ADR-0012 v1) | ✅ **deterministic dedup enforced at promotion** (shared-identifier match, hard-deny guardrails → `patient_match_review`, survivor=latest-write, merged record read-only w/ `replaced-by`, downstream reference rewrite, `patient_link`+`patient_merge_history`+merge Provenance in Gold; `FHIRENGINE_MPI=off` to disable). Splink/PPRL = external pipeline | **v2 probabilistic layer:** Fellegi-Sunter demographic scoring (Jaro-Winkler) powers `Patient/$match` (graded certain/probable/possible) and, opt-in, feeds no-shared-id candidates to the review queue — never auto-merged.
| Security (enforcement) | ✅ SMART scopes + JWKS auth, **Backend Services** (client_credentials+private_key_jwt), **UDAP B2B trust** (cert-chain software statements + **revocation: static list + live signature-verified CRL + OCSP** + **trusted DCR w/ durable registry** + **signed_metadata** + **tiered OAuth/RFC 9101 signed request** + **RFC 5280 path validation** (basic constraints / key usage / name constraints), opt-in), AuditEvent + accounting, consent + DS4P labels, obligations, **SLS auto-labeling at ingest** (ADR-0015 A2: rule-driven `meta.security` — code/prefix/value-set rules + relabel CLI); ✅ **SMART discovery** + 401/WWW-Authenticate |
